...
// encrypiton function 
char* decrypt( char *encrypted_text ,int  key, int length_of_array )
{

    int str_len=length_of_array;
    // creating  to hold characters after decryption
        
    char* array_char= malloc(sizeof(char)*str_len);
    if(array_char==NULL){printf("The memory allocation was not succefull\n");}
   
    memset(array_char,'\0',str_len); // intilizing the characte array with null characters
                                              //
    ...
    if(array== NULL ){ printf("failed to allocate positions array "); }
    memset(array,0,str_len);
    // declaration of varibles
    int total_rails=key;
    int current_rail=0;
    int current_position=0; // position of the current character
    ...
    for(int i = 0; i <total_rails ; i++)
    {
        ...
        if(current_rail == 1 || current_rail==total_rails)
        {
            ...
            {
                array_char[current_position]=encrypted_text [k];
...
// statement below show the  positions used to decrypt the cypher text 
